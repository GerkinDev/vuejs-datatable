...
import { get, Settings } from '../../classes/settings';
...
const makeVueDatatablePager = () => ( class TmpVueDatatablePager extends VueDatatablePager<TmpVueDatatablePager> {
...
describe( 'HTML content', () => {
	it( 'builds base HTML for long type', async () => {
		...
		const wrapper = mount( makeVueDatatablePager(), { localVue, propsData: { type: 'long' }} );
		await flushPromises();
	...
	it( 'builds base HTML for short type', async () => {
		get.mockImplementation( p => ( { 'pager.icons.previous': 'PREV', 'pager.icons.next': 'NEXT' } )[p] );
		localVue.prototype.$datatables = { default: { totalRows: 15, $emit: jest.fn(), pagers: [] }};
		const wrapper = mount( makeVueDatatablePager(), { localVue, propsData: { type: 'short', perPage: 5, page: 2 }} );
		await flushPromises();
	...
	it( 'builds base HTML for abbreviated type', async () => {
		...
		const wrapper = mount( makeVueDatatablePager(), { localVue, propsData: { type: 'abbreviated' }} );
		await flushPromises();
	...
	it( 'builds base HTML for abbreviated type on center page', async () => {
		get.mockImplementation( p => ( { 'pager.icons.previous': 'PREV', 'pager.icons.next': 'NEXT' } )[p] );
		localVue.prototype.$datatables = { default: { totalRows: 100, $emit: jest.fn(), pagers: [] }};
		const wrapper = mount( makeVueDatatablePager(), { localVue, propsData: { type: 'abbreviated', page: 5 }} );
		await flushPromises();
...
describe( 'Table-Pager linking', () => {
	describe( '`created`', () => {
		it( 'Pager should try to link the table on creation', () => {
			const mockLinkWithTable = jest.spyOn( VueDatatablePager.prototype, 'linkWithTable' as any );
			mount<any>( makeVueDatatablePager(), { localVue, propsData: { table: 'foo' }} );
		...
		it( 'Pager should not bind event initialization on successfull immediate linking', () => {
			jest.spyOn( VueDatatablePager.prototype, 'linkWithTable' as any ).mockReturnValue( true );
		...
		it( 'Pager should bind event initialization on failed immediate linking', () => {
			...
			const mockLinkWithTable = jest.spyOn( VueDatatablePager.prototype, 'linkWithTable' as any ).mockReturnValue( false );
			const wrapper = mount<any>( makeVueDatatablePager(), { localVue, propsData: { table: 'foo' }} );
	...
	describe( '`linkWithTable`', () => {
		it( 'Should fail if the table does not exists', () => {
			...
			const linkWithTable = VueDatatablePager.prototype['linkWithTable' as any] as ( tableName: string ) => boolean;
...
it( 'returns the correct pagination class', () => {
	...
	const wrapper = mount<any>( makeVueDatatablePager(), { localVue } as any );
...
describe( 'Pages count calculation', () => {
	it.each( [
		...
		const wrapper = mount<any>( makeVueDatatablePager(), { localVue, propsData: { perPage }} );
		/*console.log( inspect( wrapper.vm, { colors: true, depth: 1 } ) );
		console.log( inspect( wrapper.vm._computedWatchers.totalPages.getter(), { colors: true, depth: 1 } ) );
		console.log( inspect( wrapper.vm._computedWatchers.totalPages, { colors: true, depth: 1 } ) );*/
...
describe( 'properly adjusts the selected page', () => {
	it( 'properly adjusts the selected page when total pages changes', async () => {
		localVue.prototype.$datatables = { default: { totalRows: 15, $emit: jest.fn(), pagers: [] }};
		const wrapper = mount<any>( makeVueDatatablePager(), { localVue, propsData: { perPage: 2, page: 5 }} );
