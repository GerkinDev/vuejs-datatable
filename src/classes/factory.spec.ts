...
${true}       | ${true}   | ${true}         | ${true}`(
	...
	( { useTableType, transform, transformReturn, afterInstall } ) => {
		...
		if ( afterInstall ) {
			mockInstall = jest.spyOn( factory, 'installTableType' ).mockImplementation() as any;
...
${true}       | ${true}`(
	...
	( { useTableType, afterInstall } ) => {
		...
		if ( afterInstall ) {
			mockUninstall = jest.spyOn( factory, 'uninstallTableType' ).mockImplementation() as any;
			const localVue = createLocalVue();
			factory.install( localVue );
		}
...
describe( 'Use default type function', () => {
	...
	expect( factory.tableTypes as any ).toHaveProperty( 'datatable', factory.defaultTableType );
