...
import TableType, { getTableDefinition, getPagerDefinition } from './table-type';
...
it('is initialized with only the default table type registered', () => {
	jest.spyOn(Factory.prototype, 'registerTableType');
...
${true}       | ${true}   | ${true}         | ${true}`(
	...
	({ useTableType, transform, transformReturn, afterInstall }) => {
		...
		let sourceTableType: TableType;
		if (useTableType) {
			sourceTableType = new TableType(name);
		}

		let tableTypeSave: TableType;
		let mockTransform: jest.Mock<any, [any]>;
		if (transform) {
		...
		let mockInstall: jest.Mock<any, [any]>;
		if (afterInstall) {
			mockInstall = jest.spyOn(factory, 'installTableType').mockImplementation();
		...
		expect(factory.tableTypes).toHaveProperty(name);
...
${true}       | ${true}`(
	...
	({ useTableType, afterInstall }) => {
		...
		let mockUninstall: jest.Mock<any, [any]>;
		if (afterInstall) {
			mockUninstall = jest.spyOn(factory, 'uninstallTableType').mockImplementation();
			const localVue = createLocalVue();
			factory.install(localVue);
		}
		...
		expect(factory.tableTypes).not.toHaveProperty(name);
		if (afterInstall) {
	...
	it('Deregistered table won\'t be installed on factory installation', () => {
		...
		jest.spyOn(factory, 'installTableType').mockImplementation();
		jest.spyOn(factory, 'uninstallTableType').mockImplementation();
